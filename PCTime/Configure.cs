...
namespace PCTime
{
    public partial class Configure : Form
    {
        ...
        string shortcutPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Startup), "PCTime.lnk");
            ...
            try
            {
                if (!checkBox_enableTimer.Checked)
                {
                    if (MessageBox.Show(Program.rm_Messages.GetString("notEnableTimerConfirmation_text"),
                        "PCTime", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
                    {
                        return;
                    }
                }
                ...
                if (checkBox_startPCTimeWithWindows.Checked && !File.Exists(shortcutPath) && !Program.debug)
                {
                    try
                    {
                    ...
                    catch (MissingMethodException ex)
                    {
                        Program.logger.Log(className, "Error while creating the shortcut in the startup folder: " +
                            "Your .NET version does not support this function. Please make a shortcut manually for PCTime.exe in the startup folder (" +
                        ...
                        MessageBox.Show("Error while creating the shortcut in the startup folder: " +
                            ...
                            "PCTime", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                    catch (Exception ex)
                    {
                ...
                else if (!checkBox_startPCTimeWithWindows.Checked && File.Exists(shortcutPath) && !Program.debug)
                {
                    try
                    {
        ...
        private void Configure_Load(object sender, EventArgs e)
        {
            ...
            Program.rm_Configure = new ResourceManager(string.Format("PCTime.Languages.{0}.Configure", Program.selectedLanguage), Program.assembly);
            ...
            checkBox_startPCTimeWithWindows.Text = Program.rm_Configure.GetString("checkBox_startPCTimeWithWindows_text");
            ...
            checkBox_startPCTimeWithWindows.Checked = File.Exists(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Startup),
                    "PCTime.lnk"));
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show(Program.rm_Messages.GetString("resetConfirmation"),
                "PCTime", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                Program.logger.Log(className, "Resetting PCTime...", Logger.LogType.Info);
