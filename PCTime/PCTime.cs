...
namespace PCTime
{
    public partial class PCTime : Form
    {
        private const string className = "PCTime.cs"; //Used for logging
        ...
        private void Form1_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (e.CloseReason == CloseReason.UserClosing && !instantExit)
            {
                ...
                if (Properties.Settings.Default.closeNoticeFirstTime)
                {
                    if (timer.Enabled)
                    {
                        MessageBox.Show(Program.rm_Messages.GetString("closeNotice"), "PCTime", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                    else
                    {
                        MessageBox.Show(Program.rm_Messages.GetString("closeNotice_v2"), "PCTime", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
            ...
            else
            {
                ...
                Program.logger.Log(className, "executing PCTime closing event", Logger.LogType.Info);

                Program.logger.Log(className, "Quitting PCTime", Logger.LogType.Info);
        ...
        private void PCTime_Load(object sender, EventArgs e)
        {
            GUI_Load();
        }
        ...
        public void GUI_Load()
        {
            ...
            {
                Program.rm_GUI = new ResourceManager(string.Format("PCTime.Languages.{0}.GUI", Program.selectedLanguage), Program.assembly);
            }
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            {
                PauseTimer(true);
                Program.notifyIcon1.ShowBalloonTip(2500, "PCTime", Program.rm_Messages.GetString("timerPausedNotification_text"), ToolTipIcon.Info);
            }
            else
            {
                ResumeTimer(true);
                Program.notifyIcon1.ShowBalloonTip(2500, "PCTime", Program.rm_Messages.GetString("timerResumedNotification_text"), ToolTipIcon.Info);
            }
        }
        ...
        private void timer_Tick(object sender, EventArgs e)
        {
            ...
            {
                Program.notifyIcon1.ShowBalloonTip(10000, "PCTime", Program.rm_Messages.GetString("tenMinutesRemainingNotification_text"),
                        ToolTipIcon.Info);
            }
            ...
            if (!overtimeC && !overtimeY && secondsElapsed >= maximumSeconds)
            {
                if (timesOutEvent)
                {
                    ...
                    Program.notifyIcon1.ShowBalloonTip(10000, "PCTime", Program.rm_Messages.GetString("timeIsOutNotification_text"),
                        ToolTipIcon.Info);
                    timesOutEvent = false;
                    overtimeC = true;
                }
            ...
            if (overtimeC && maximumSecondsOvertime > 0 && secondsElapsed >= maximumSeconds + maximumSecondsOvertime)
            {
                if (timesOutOvertimeEvent)
                {
                    ...
                    Program.notifyIcon1.ShowBalloonTip(10000, "PCTime", Program.rm_Messages.GetString("overtimeIsOutNotification_text"),
                        ToolTipIcon.Info);
        ...
        {
            if (MessageBox.Show(Program.rm_Messages.GetString("quitConfirmationDialog_text"), "PCTime",
        ...
        private void PCTime_HelpButtonClicked(object sender, System.ComponentModel.CancelEventArgs e)
        {
            e.Cancel = true;
            if (MessageBox.Show("Open GitHub page?", "PCTime", MessageBoxButtons.YesNo, MessageBoxIcon.Question)
                == DialogResult.Yes)
            {
