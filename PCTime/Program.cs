...
namespace PCTime
{
    static class Program
    {
        ...
        public static PCTime pctimeForm;
        ...
        static void Main(string[] args)
        {
            ...
            debug = true;
#endif
            string logDir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "PCTime");
            ...
            if (debug)
            {
                logDir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "PCTime", "debug");
                if (!Directory.Exists(logDir))
                {
                    Directory.CreateDirectory(logDir);
                }
            ...
            logger = new Logger("PCTime", Path.Combine(logDir, "log.txt"),
                Logger.LogTimeDateOptions.YearMonthDayHourMinuteSecond, true);

            bool createdNew = true;
            using (Mutex mutex = new Mutex(true, "PCTime", out createdNew))
            {
                ...
                else
                {
                    ...
                    //Bring the window of the already running instance of PCTime to the front
                    try
                    {
                    ...
                    {
                        MessageBox.Show("Error: " + ex.Message + "\r\n\r\n\r\nFull error details: " + ex, "PCTime", MessageBoxButtons.OK,
        ...
        static void StartApplication(string[] args)
        {
            ...
            logger.Log(className, "Starting PCTime", Logger.LogType.Info);
            ...
            rm_Messages = new ResourceManager(string.Format("PCTime.Languages.{0}.Messages", selectedLanguage), assembly);
            rm_GUI = new ResourceManager(string.Format("PCTime.Languages.{0}.GUI", selectedLanguage), assembly);
            ...
            {
                logger.Log(className, "Resetting PCTime", Logger.LogType.Info);

                Properties.Settings.Default.Reset();
                Properties.Settings.Default.upgradeRequired = false;
                Properties.Settings.Default.Save();

                logger.Log(className, "Reset completed, closing PCTime", Logger.LogType.Info);
            ...
            {
                MessageBox.Show("ERROR: " + ex.Message + "\r\n\r\n\r\nFull error details: " + ex, "PCTime", MessageBoxButtons.OK, MessageBoxIcon.Error);
                logger.Log(className, Convert.ToString(ex), Logger.LogType.Error);
                Application.Exit();
                return;
            }
        ...
        {
            if (!confirmMsg || MessageBox.Show(rm_Messages.GetString("removalQuestion"), "PCTime",
        ...
        public static void LoadNotifyIcon()
        {
            ...
            if (!debug)
            {
                notifyIcon1.Text = "PCTime";
            }
            else
            {
                notifyIcon1.Text = "PCTime DEBUG";
            }
        ...
        static void PauseResumeTimer(object sender, EventArgs e)
        {
            ...
            if (pctimeForm.timer.Enabled)
            {
                ...
                notifyIcon1.ShowBalloonTip(2500, "PCTime", rm_Messages.GetString("timerPausedNotification_text"), ToolTipIcon.Info);
            }
            else
            {
                pctimeForm.ResumeTimer(updateGUI);
                notifyIcon1.ShowBalloonTip(2500, "PCTime", rm_Messages.GetString("timerResumedNotification_text"), ToolTipIcon.Info);
            }
        }

        static void Quit(object sender, EventArgs e)
        {
            if (MessageBox.Show(rm_Messages.GetString("quitConfirmationDialog_text"), "PCTime", MessageBoxButtons.YesNo,
