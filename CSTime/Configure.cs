...
namespace CSTime
{
    public partial class Configure : Form
    {
        ...
        string shortcutPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Startup), "CSTime.lnk");
            ...
            try
            {
                if (!checkBox_enableTimer.Checked)
                {
                    if (MessageBox.Show(Program.rm_Messages.GetString("notEnableTimerConfirmation_text"),
                        "CSTime", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
                    {
                        return;
                    }
                }
                ...
                if (checkBox_startCSTimeWithWindows.Checked && !File.Exists(shortcutPath) && !Program.debug)
                {
                    try
                    {
                    ...
                    catch (MissingMethodException ex)
                    {
                        Program.logger.Log(className, "Error while creating the shortcut in the startup folder: " +
                            "Your .NET version does not support this function. Please make a shortcut manually for CSTime.exe in the startup folder (" +
                        ...
                        MessageBox.Show("Error while creating the shortcut in the startup folder: " +
                            ...
                            "CSTime", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                    catch (Exception ex)
                    {
                ...
                else if (!checkBox_startCSTimeWithWindows.Checked && File.Exists(shortcutPath) && !Program.debug)
                {
                    try
                    {
        ...
        private void Configure_Load(object sender, EventArgs e)
        {
            ...
            checkBox_startCSTimeWithWindows.Text = Program.rm_Configure.GetString("checkBox_startCSTimeWithWindows_text");
            ...
            checkBox_startCSTimeWithWindows.Checked = File.Exists(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Startup),
                    "CSTime.lnk"));
        }
